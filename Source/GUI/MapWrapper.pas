...
uses
  ...
  Winapi.GDIPOBJ, Vcl.Graphics, Vcl.Imaging.pngimage, Vcl.Imaging.jpeg, Vcl.Controls,
  LocalMap, FilesMonitor, App.Constants, ResUIWrapper, SimpleLogger;
...
type
  ...
  public
    ...
    procedure LoadMap(const Map: TLocalMap);
    ...
    property Map: TLocalMap read FMap;
...
procedure TMapWrapper.DrawMapTags(Bitmap: TBitmap);
  ...
  procedure DrawTag(ico: TPNGImage; const Caption: string; Position: TPoint);
  var
    Offset: Double;
  begin
...
procedure TMapWrapper.LoadMap(const Map: TLocalMap);
var
  Jpeg: TJPEGImage;
begin
  FMap := Map;
  ...
  try
    Jpeg.LoadFromFile(TPath.Combine(AppParams.MapsPath, Map.FileName));
...
//    bmp.Assign(FBackground);
    ...
    if not Value.Empty then begin
      Offset := Abs((FMap.Left.X - Value.X) / (FMap.Right.X - FMap.Left.X));
      p.X := Trunc(bmp.Width * Offset);
      Offset := Abs((FMap.Left.Y - Value.Y) / (FMap.Right.Y - FMap.Left.Y));
